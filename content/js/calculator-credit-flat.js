	...
	function change_calc() {
		...
		var reg_c = parseInt(data_calc[0].value);
		...
		var ocenka_c = data_calc[8].value;

		var error_c = 0;

		if ($.isNumeric(sum_c)) {
			if ([16, 18, 19, 32].indexOf(reg_c) != -1) { // Мск и обл. и Спб и обл.
				if (sum_c < 1000000) {
					$(".credit input[name='sum']").addClass('error');
					sum_c = 1;
					error_c = 1;
				} else if (sum_c > 10000000) {
					$(".credit input[name='sum']").addClass('error');
					sum_c = 1;
					error_c = 1;
				} else {
			...
			$(".credit input[name='sum']").val(1000000);
			sum_c = 1000000;
			error_c = 1;
		}
		...
		if (ocenka_c < sum_c / 0.7) {
			$(".credit input[name='ocenka']").addClass('error');
			$(".notation_ocenka").html("Сумма кредита не может превышать 70% оценочной стоимости квартиры");
			error_c = 1;
		} else {
			$(".credit input[name='ocenka']").removeClass('error');
			$(".notation_ocenka").html("");
		}
